...
<?php $elearning_education_column_layout = get_theme_mod( 'elearning_education_sidebar_post_layout');
if($elearning_education_column_layout == 'four-column' || $elearning_education_column_layout == 'three-column' ){ ?>
        ...
        <?php if (!is_single() && function_exists('get_post_gallery')) {
          ...
          if ($gallery && isset($gallery['ids'])) { 
            ...
            <div class="container entry-gallery">
              <div class="row">
                <?php $elearning_education_max_images = min(count($gallery_ids), 4);
                for ($index = 0; $index < $elearning_education_max_images; $index++) {
                  $id = $gallery_ids[$index];
                  $elearning_education_image_url = wp_get_attachment_image_url($id, 'full'); ?>
                  <div class="col-md-6 mb-1 align-self-center">
                    <img class="img-fluid" src="<?php echo esc_url($elearning_education_image_url); ?>" alt="Gallery Image <?php echo ($index + 1); ?>">
                  </div>
                <?php } ?>
              </div>
            </div>
...
<div id="category-post">
  <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <div class="page-box row">
      <?php $elearning_education_post_layout = get_theme_mod( 'elearning_education_post_layout','image-content');
      if($elearning_education_post_layout == 'image-content'){ ?>
      ...
      <?php }
      else if($elearning_education_post_layout == 'content-image'){ ?>
        <div class="box-content col-lg-6 col-md-12">
          <h4><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php the_title_attribute(); ?>"><?php the_title();?></a></h4>
          <div class="box-info">
            <?php $elearning_education_blog_archive_ordering = get_theme_mod('blog_meta_order', array('date', 'author', 'comment', 'category'));
